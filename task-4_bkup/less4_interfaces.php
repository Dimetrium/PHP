...
подключение к базе
mysql user5 -uuser5 -ptuser5
exit: \q
query ends ';' (\G это для селектов - она меняет форму отображения информацию)
create table
...
В инетрфейсах прописаны сколько у нас есть обяз. методы.
мы должны добавить какие то методы которых нету...(если есть гетеры должны быть сетеры)
глав обьект - груупа
в ней музыканты(с именами) и с обектами на чем играют.

это паттерн проэктирования (композиция)

потом создать обект группы. (с умзыкантами и инстурментами)
потом надо распечатать при помощи форича.
